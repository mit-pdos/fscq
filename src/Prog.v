...
Notation "^( a , b , .. , c )" := (pair b .. (pair c a) .. ).
...
Notation "'let^' ( a , b , .. , c ) <- p1 ; p2" :=
  (progseq p1
    (pair_args_helper (fun b => ..
      (pair_args_helper (fun c a => p2))
    ..))
  )
  (at level 60, right associativity, b closed binder, c closed binder).
