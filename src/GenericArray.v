...
Fixpoint array {V : Type} (a : word sz) (vs : list V) (stride : word sz) : @pred _ (@weq sz) _ :=
  match vs with
    | nil => emp
...
Definition sel (V : Type) (vs : list V) (i : word sz) (default : V) : V :=
...
Definition upd T (vs : list T) (i : word sz) (v : T) : list T :=
  updN vs (wordToNat i) v.

Definition upd_prepend (vs : list valuset) (i : word sz) (v : valu) : list valuset :=
  upd vs i (v, valuset_list (sel vs i ($0, nil))).

Definition upd_sync (vs : list valuset) (i : word sz) (default : valuset) : list valuset :=
...
Theorem isolate_fwd : forall V (default : V) (a i : word sz) vs stride,
  wordToNat i < length vs
...
Theorem isolate_bwd : forall V (default : V) (a i : word sz) vs stride,
  wordToNat i < length vs
...
Theorem array_isolate : forall V (default : V) (a i : word sz) vs stride,
  wordToNat i < length vs
  -> array a vs stride <=p=>
     array a (firstn (wordToNat i) vs) stride
     * (a ^+ i ^* stride) |-> sel vs i default
...
Theorem array_isolate_upd : forall V (v : V) (a i : word sz) vs stride,
  wordToNat i < length vs
...
Theorem isolate_bwd_upd : forall V (v : V) (a i : word sz) vs stride,
  wordToNat i < length vs
  -> array a (firstn (wordToNat i) vs) stride
     * (a ^+ i ^* stride) |-> v
     * array (a ^+ (i ^+ $1) ^* stride) (skipn (S (wordToNat i)) vs) stride
...
Lemma array_app_memupd : forall V l (v : V) m (b : word sz),
...
Proof.
  ...
  assert (wordToNat (natToWord sz (length l)) = length l).
...
Theorem array_max_length : forall T (l : list T) m start stride,
  array start l stride m -> goodSizeEq sz (length l).
Proof.
  unfold goodSizeEq; intros.
  setoid_rewrite <- (firstn_skipn (pow2 sz)) in H.
  pose proof (@array_app_stride T (firstn (pow2 sz) l)
                                  (skipn (pow2 sz) l)
                                  start
                                  (start ^+ $ (pow2 sz) ^* stride)
                                  stride).
  destruct (le_dec (length l) (pow2 sz)); auto.
  destruct H0.
  rewrite firstn_length. rewrite Min.min_l by omega. auto.
  apply H1 in H. clear H0 H1.
  rewrite natToWord_pow2 in *.
Admitted.

Theorem array_max_length_F : forall T (l : list T) m start stride F,
  (F * array start l stride)%pred m -> goodSizeEq sz (length l).
Proof.
...
Theorem array_max_length_pimpl : forall T (l : list T) start stride,
  array start l stride =p=> array start l stride * [[ goodSizeEq sz (length l) ]].
Proof.
  unfold pimpl; intros.
  pred_apply; cancel.
...
Lemma isolate_last: forall A l (a : A) (b:word sz),
  length l <= wordToNat b ->
  (array $0 (l ++ a :: nil) $1 <=p=>
   array $0 l $1 * $ (length l) |-> a)%pred.
Proof.
  intros.
  assert (wordToNat (natToWord sz (length l)) = length l) as Heq by
