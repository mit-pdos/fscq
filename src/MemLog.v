...
Module MEMLOG.
 ...
 Qed.
    ...
    {{fun done_ crash_ =>
      exists m1 xp F,
        (F * [[ cachesize <> 0 ]] * crash_xform (would_recover_old xp m1) *
          [[forall r_ : memstate * cachestate * fs_xparams,
            {{fun (done'_ : donecond T) (crash'_ : pred) =>
              F *
              (rep (FSXPMemLog (snd r_)) (NoTransaction m1) (fst r_)) * 
        ...
        exists m2, (F * [[ cachesize <> 0 ]] * crash_xform (would_recover_either xp m1 m2) *
          [[forall r_ : memstate * cachestate * fs_xparams,
            {{fun (done'_ : donecond T) (crash'_ : pred) =>
              F *
              (rep (FSXPMemLog (snd r_)) (NoTransaction m1) (fst r_) \/
               rep (FSXPMemLog (snd r_)) (NoTransaction m2) (fst r_)) * 
              [[done'_ = done_]] * [[crash'_ = crash_]]}} rx r_]] * 
  ...
  Variable main_prog : forall T, memstate * Cache.cachestate -> (memstate * Cache.cachestate * bool -> prog T) -> prog T.

  Theorem recover_corr2_to_corr3: forall mscs (pre post: pred) xp cachesize,
    {< mbase,
    PRE             rep xp (NoTransaction mbase) mscs * 
                    [[ pre (list2mem mbase)]]
    POST:(mscs', r) [[ r = false ]] * rep xp (NoTransaction mbase) mscs' \/
    ...
    POST:(mscs', r) [[ r = false ]] * rep xp (NoTransaction mbase) mscs' \/
                    [[ r = true ]] * exists m', rep xp (NoTransaction m') mscs' *
                    [[ post (list2mem m') ]]
    CRASH:r         (rep (snd r).(FSXPMemLog) (NoTransaction mbase) (fst r) \/
                    exists m', rep (snd r).(FSXPMemLog) (NoTransaction m') (fst r) *
                    [[ post (list2mem m') ]])
