...
Ltac extract_var_val :=
  ...
  | [ H: (?P * any)%pred (mem_of ?s) |- _ ] => extract_var_val_from H P s
  end.
...
Proof.
  ...
  - eapply hoare_weaken; [ apply H | cancel_subset.. ].
...
Proof.
  ...
  cancel_subset. intro; simpl. eapply chop_any. subst F.
  match goal with
  | [ |- ?l =p=> ?r ] => set (l_ := l)
  end.
  pattern x0 in l_.
