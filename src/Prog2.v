...
Inductive mem_prog : Type -> Type :=
  | Read (a: addr) : mem_prog valu
  | Write (a: addr) (v: valu) : mem_prog unit
  | Trim (a: addr) : mem_prog unit.

Inductive var_prog : Type -> Type :=
  | VarAlloc (T : Type) (v : T) : var_prog vartype
  | VarDelete (i : vartype) : var_prog unit
  | VarGet (i : vartype) (T : Type) : var_prog T
  | VarSet (i : vartype) (T : Type) (v : T) : var_prog unit.
...
Inductive prog : Type -> Type :=
  | Ret T (v: T) : prog T
  | Mem T (p: @mem_prog T) : prog T
  | Var T (p: @var_prog T) : prog T
  | Sync : prog unit
...
Inductive mem_step : forall T,
    rawdisk -> varmem -> hashmap -> mem_prog T ->
    rawdisk -> varmem -> hashmap -> T -> Prop :=
    
| StepRead : forall m a v x vm hm,
    m a = Some (v, x) ->
    mem_step m vm hm (Read a) m vm hm v
| StepWrite : forall m a v v0 x vm hm,
    m a = Some (v0, x) ->
    mem_step m vm hm (Write a v) (upd m a (v, v0 :: x)) vm hm tt
| StepTrim : forall m a vs vs' vm hm,
    m a = Some vs ->
    mem_step m vm hm (Trim a) (upd m a vs') vm hm tt.


Inductive var_step : forall T,
    rawdisk -> varmem -> hashmap -> var_prog T ->
    rawdisk -> varmem -> hashmap -> T -> Prop :=
    
| StepVarAlloc : forall T d v vm i hm,
    vm i = None ->
    var_step d vm hm (@VarAlloc T v) d (insert vm i (@Any T v)) hm i
| StepVarDelete : forall d v vm i hm,
    vm i = Some v ->
    var_step d vm hm (VarDelete i) d (delete vm i) hm tt
| StepVarGet : forall T d v vm i hm,
    vm i = Some (@Any T v) ->
    var_step d vm hm (@VarGet i T) d vm hm v
| StepVarSet : forall T d v0 v vm i hm,
    vm i = Some v0 ->
    var_step d vm hm (@VarSet i T v) d (upd vm i (@Any T v)) hm tt.

Inductive step : forall T,
    rawdisk -> varmem -> hashmap -> prog T ->
    rawdisk -> varmem -> hashmap -> T -> Prop :=
| StepMem : forall T m vm hm (p: mem_prog T) m' r,
    mem_step m vm hm p m' vm hm r ->
    step m vm hm (Mem p) m' vm hm r
| StepVar : forall T m vm hm (p: var_prog T) vm' r,
    var_step m vm hm p m vm' hm r ->
    step m vm hm (Var p) m vm' hm r
...
Inductive fail_mem_step : forall T,
    rawdisk -> varmem -> mem_prog T -> Prop :=
| FailRead : forall m vm a,
    m a = None ->
    fail_mem_step m vm (Read a)
| FailWrite : forall m vm a v,
    m a = None ->
    fail_mem_step m vm (Write a v)
| FailTrim : forall m vm a,
    m a = None ->
    fail_mem_step m vm (Trim a).
    
Inductive fail_var_step : forall T,
    rawdisk -> varmem -> var_prog T -> Prop :=
| FailVarDelete : forall m vm i,
    vm i = None ->
    fail_var_step m vm (VarDelete i)
| FailVarGetType : forall T T' m vm i (v : T'),
    T <> T' ->
    vm i = Some (Any v) ->
    fail_var_step m vm (@VarGet i T)
| FailVarGetNone : forall T m vm i,
    vm i = None ->
    fail_var_step m vm (@VarGet i T)
| FailVarSetNone : forall T m vm i (v : T),
    vm i = None ->
    fail_var_step m vm (VarSet i v).


Inductive fail_step : forall T,
    rawdisk -> varmem -> prog T -> Prop :=
| FailMem : forall T m vm (p: mem_prog T),
    fail_mem_step m vm p ->
    fail_step m vm (Mem p)
| FailVar : forall T m vm (p: var_prog T),
    fail_var_step m vm p ->
    fail_step m vm (Var p).

Inductive crash_mem_step : forall T, mem_prog T -> Prop :=
| CrashRead : forall a,
    crash_mem_step (Read a)
| CrashWrite : forall a v,
    crash_mem_step (Write a v).
    
    
Inductive crash_step : forall T, prog T -> Prop :=
| MemCrash : forall T (p: mem_prog T),
    crash_mem_step p ->
    crash_step (Mem p).
