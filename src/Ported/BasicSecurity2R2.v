...
Definition prog_secure AT AEQ V T 
  (wc: @mem addr addr_eq_dec valuset -> @mem AT AEQ V -> Prop)
  (wr: @mem addr addr_eq_dec valuset -> @mem AT AEQ V -> Prop)
...
Theorem bind_secure:
  ...
  -> (forall x, prog_secure wr1 wr2 (p2 x) post1 post2)
...
Theorem world_impl_secure:
  ...
  -> (forall m m', post m' -> wr1 m m' -> wr2 m m')
...
Theorem cross_world_secure_weak:
  ...
  -> (forall m ml, wrl m ml -> exists mh, conv ml mh /\ wrh m mh)
...
Theorem prog_secure_is_noninterference :
  forall T AT AEQ V (p : prog T) (wc wr: @mem addr addr_eq_dec valuset -> @mem AT  AEQ V -> Prop) pre post a v1 v2 m1 m2 m mp vm hm,
  prog_secure wc wr p pre post ->
...
Proof.
  ...
  - simpl; intros; split_hyp; repeat deex.
    ...
    + eexists; intuition; eauto.
      left; auto.
      instantiate (1:=(fun m (m': @mem addr addr_eq_dec file) => 
      exists arr' fr', (L1.rep arr' * [[ L2.rep fr' (arr2file arr') ]]
      * [[ (arrayN_ex (ptsto (V:=file)) fr fn 
      * diskIs m')%pred (list2nmem fr')]] )%pred m)).
      simpl.
      rewrite (diskIs_pred _ H5) in H10.
      apply arrayN_ex_ptsto_merge in H10.
      apply list2nmem_array_mem_eq in H10; subst.
      eexists; pred_apply; cancel.
      unfold rep in *.
      apply list2nmem_inj in H10.
    ...
    + exists (singleton addr_eq_dec fn (L2.upd_file (selN fr fn fdef) bn v (v,nil))).
      ...
      apply in_updN in H9; destruct H9.
      eauto.
      unfold upd_file in H9.
      rewrite H9; simpl; rewrite length_updN.
      apply H14.
      apply in_selN; auto.
      admit. (* tedious but doable *)
  ...
  - simpl; intros; split_hyp; eauto.
    destruct_lift H.
    destruct_lift H1.
    eapply list2nmem_arrayN_ex_diskIs_ptsto; eauto.
    eapply lt_le_trans.
    2: eapply arr2file_length; eauto.
    simpl; instantiate (1:= fn).
    rewrite Nat.mul_succ_r; omega.
