...
Module DIRTREE.
  ...
  Fixpoint find_inum inum (tree:dirtree) : list (list string) :=
    match tree with
    | TreeFile inum' _ => if addr_eq_dec inum inum' then [ [] ] else []
    | TreeDir inum' ents =>
      (if addr_eq_dec inum inum' then [ [] ] else [])
        ++ (concat (map (fun ent =>
                           let '(name, item) := ent in
                           map (cons name) (find_inum inum item)) ents))
    end.

  Theorem find_inum_inums : forall inum tree path,
      In path (find_inum inum tree) ->
      exists t, find_subtree path tree = Some t /\
           dirtree_inum t = inum.
  Proof.
  Abort.
