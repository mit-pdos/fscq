...
(**
 * A helpful theorem and tactic for dealing with the [let] statements
 * generated by [compute_rec].
 *)
Theorem let_fst_snd : forall (A B R : Type) (p : A * B) (f : A -> B -> R),
  (let (a, b) := p in f a b) = (f (fst p) (snd p)).
Proof.
  destruct p; auto.
Qed.

Ltac rec_simpl := cbn; repeat ( rewrite let_fst_snd; cbn ).
