...
Definition copy_block T xp (a b : addr) mscs rx : prog T :=
  let^ (mscs, v) <- MEMLOG.read xp a mscs;
...
Definition testalloc T lxp bxp rx : prog T :=
  ...
  | Some bn =>
    ...
    let^ (mscs, ok) <- MEMLOG.commit lxp mscs ;
    rx (Some bn)
  end.

Definition test_bfile T fsxp v mscs rx : prog T :=
  mscs <- MEMLOG.begin (FSXPMemLog fsxp) mscs ;
  let^ (mscs, ok) <- BFILE.bfgrow (FSXPMemLog fsxp) (FSXPBlockAlloc fsxp) (FSXPInode fsxp) $3 mscs ;
  match ok with
  | false =>
  ...
  | true =>
    ...
    | true =>
      mscs <- MEMLOG.begin (FSXPMemLog fsxp) mscs ;
      let^ (mscs, b) <- BFILE.bfread (FSXPMemLog fsxp) (FSXPInode fsxp) $3 $0 mscs ;
