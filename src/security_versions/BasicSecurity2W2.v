...
Theorem frame_lift_empty_secure:
  forall AT AEQ V T w (p : prog T) (pre post crash: @pred AT AEQ V) (P P' P'': Prop),
  P ->
  prog_secure w p ([[ P ]] * pre)%pred ([[ P' ]] * post)%pred ([[ P'' ]] * crash)%pred ->
  prog_secure w p pre post crash.
Proof.
  intros.
  eapply pimpl_secure.
...
Theorem cross_world_secure:
  ...
  inverse conv conv_inv
  -> cross_same_world wl wh conv conv_inv
...
Proof.
  unfold corresponds, corresponds',
    cross_same_world, inverse, prog_secure; intros.
  split_hyp.
  sep_unfold H3.
  sep_unfold H4.
  ...
  - intuition; repeat deex.
    left.
    eapply H7 in H19; eauto.
    do 6 eexists; intuition; eauto.
    rewrite H0; eauto.
    rewrite H0; eauto.
  - intuition; repeat deex.
    right.
    eapply H8 in H19; eauto.
    repeat eexists; eauto.
    rewrite H0; eauto.
    rewrite H0; eauto.
Qed.
...
Theorem read_data_secure:
  forall arr a vdef vsdef,
  @prog_secure _ addr_eq_dec valuset _ 
  (fun m => (exists arr F, ([[[ (Data arr) ::: (F * diskIs m)%pred ]]] * (L1.rep arr)))%pred) (L1.read_data arr a vdef) (a |-> (selN (Data arr) a vsdef))%pred (a |-> (selN (Data arr) a vsdef))%pred (a |-> (selN (Data arr) a vsdef))%pred.
Proof.
  ...
  all: eauto.
  instantiate (1:= (a |-> (selN (Data arr) a vsdef))%pred).
  cancel.
...
(* 
  ...
  - rewrite (diskIs_pred _ H) in H0.
    instantiate (1:= (exists arr, (arrayN (ptsto (V:=valuset)) 0 (firstn a (Data arr)) * arrayN (ptsto (V:=valuset)) (a + 1)
       match Data arr with
       | nil => nil
       | _ :: l => skipn a l
       end)%pred)%pred) in H0.
    destruct_lift H0.
    exists {| L1.Data := (firstn a (Data dummy)++((selN (Data arr) a (pair a0 b)) :: skipn (S a) (Data dummy))) |}.
    pred_apply; cancel.
    unfold pimpl; intros.
    apply arrayN_app; simpl.
    eapply isolateN_bwd with (i:= a); auto.
    rewrite app_length; simpl.
    pred_apply; cancel.
    sep_unfold H0.
    eapply ptsto_complete in H; eauto; subst.
    unfold_sep_star; repeat eexists.
    instantiate (1:= (mem_union m0 m3)).
    admit.
    eauto.
  ...
  - destruct_lift H0.
    eapply isolateN_fwd with (i:= a) in H0; auto.
    pred_apply; cancel.
    unfold pimpl; intros.
  ...
  - rewrite (diskIs_pred _ H0) in H1.
    ...
    eapply isolateN_bwd with (i:= a); auto.
    pred_apply; cancel.
    eapply list2nmem_mem_diff_diskIs; eauto. 
  
  - destruct_lift H1.
    eapply isolateN_fwd with (i:= a) in H1; auto.
    pred_apply; cancel.
    unfold pimpl; intros.
    eapply ptsto_complete; eauto.
...
Theorem read_from_file_secure':
  forall arr fr fn bn fdef vsdef (f: L2.file),
     @prog_secure _ addr_eq_dec _ _
      (fun m => (exists arr fr F, ([[(F * diskIs m)%pred (list2nmem fr)]] * [[ L2.rep fr (arr2file arr)]] * L1.rep arr)%pred)%pred)
...
Proof.
    ...
    instantiate (1:= (fn |-> f)%pred).
    cancel.
    eapply cross_world_secure.
    4: eapply pimpl_secure.
    7: eauto.

    Focus 2.
    unfold cross_same_world.
    intuition; split; cancel; eauto.
    admit.
    admit.
    unfold rep.
    3: apply H4.
    eapply list2nmem_mem_diff_diskIs.
    
    (* Need info about mh and ml *)
    admit.
    admit.
