...
Inductive progop : Set -> Type :=
| OpRead (a: addr) : progop valu
| OpWrite (a: addr) (v: valu) : progop unit.

Inductive prog : Set -> Type :=
| Return_ (R: Set) (v: R) : prog R
| Cont_ (R S: Set) (op: progop R) (rx: R->prog S) : prog S.

Notation "'Return'" := Return_.
Notation "!" := OpRead.
Infix "<--" := OpWrite (at level 8).
Notation "p1 ;; p2" := (Cont_ (p1) (fun _: unit => p2)) (at level 9, right associativity).
Notation "x <- p1 ; p2" := (Cont_ (p1) (fun x => p2)) (at level 9, right associativity).
...
| Returned (v : R)
| Crashed.
Implicit Arguments Returned [R].
Implicit Arguments Crashed [R].

Inductive opstep : forall (R: Set), mem -> progop R -> mem -> R -> Prop :=
| StepRead: forall m a,
  opstep m (OpRead a) m (m a)
| StepWrite: forall m a v,
  opstep m (OpWrite a v) (upd m a v) tt.

Inductive exec : forall (R: Set), mem -> prog R -> mem -> outcome R -> Prop :=
| XReturn: forall (R: Set) m (v: R),
  exec m (Return v) m (Returned v)
| XStep: forall (R S: Set) m op m' (r:R) (rx:R->prog S) m'' out,
  opstep m op m' r ->
  exec m' (rx r) m'' out ->
  exec m (Cont_ op rx) m'' out
| XCrash: forall (R: Set) m (p: prog R),
  exec m p m Crashed.
