...
Section GenArray.
  ...
  Proof.
    ...
    destruct i; rewrite <- H1 in *; simpl.
    ...
    destruct i; rewrite <- H0 in *; simpl.
    replace (a0 + 0) with (a0) by omega.
    replace (a0 + 1) with (S a0) by omega.
    cancel.
...
Definition vsupd vs i v :=  updN vs i (v, vsmerge (selN vs i valuset0)).
Definition vssync (vs: list valuset) i :=  updN vs i (fst (selN vs i valuset0), nil).

Definition vsupd_range (vsl : list valuset) vl :=
  let n := length vl in
  (List.combine vl (map vsmerge (firstn n vsl))) ++ skipn n vsl.

Definition vs_synced a (vl : list valuset) :=
  snd (selN vl a valuset0) = nil.
...
Lemma vsupd_range_progress : forall i vsl l,
  length l <= length vsl -> i < length l ->
    (vsupd (vsupd_range vsl (firstn i l)) i (selN l i tagged_block0))
  = (vsupd_range vsl ((firstn i l) ++ [ selN l i tagged_block0 ])).
Proof.
...
Lemma vsupd_range_selN_oob : forall vs n l,
  n >= length l ->
  length l <= length vs ->
  selN (vsupd_range vs l) n valuset0 = selN vs n valuset0.
Proof.
  unfold vsupd_range; intros.
  rewrite selN_app2.
...
Lemma vsupd_range_selN_inb : forall vs n l,
  n < length l ->
  length l <= length vs ->
  selN (vsupd_range vs l) n valuset0 = (selN l n tagged_block0, vsmerge (selN vs n valuset0)).
Proof.
  unfold vsupd_range; intros.
  rewrite selN_app1.
  unfold valuset0 in *.
  ...
  rewrite map_length, firstn_length_l; auto; omega.
  rewrite combine_length_eq; auto.
  rewrite map_length, firstn_length_l; omega.
Qed.
...
Proof.
  ...
  rewrite <- updN_selN_eq with (ix := n) (l := vs) (default := valuset0) at 2.
  apply forall2_updN.
  apply IHn; omega.
...
Definition vsupsyn_range (vsl : list valuset) vl :=
  let n := length vl in
...
Definition vsupd_vecs (vsl : list valuset) (l : list (addr * tagged_block)) : list valuset :=
...
Lemma vsupd_vecs_progress : forall l m vs,
  m < length l ->
  let e := selN l m (0, tagged_block0) in
...
Lemma vsupd_vecs_selN_not_in : forall l a d,
  ~ In a (map fst l) ->
  selN (vsupd_vecs d l) a valuset0 = selN d a valuset0.
Proof.
...
Lemma vsupd_vecs_selN_vsmerge_in' : forall a v avl l,
  In v (vsmerge (selN l a valuset0)) ->
  a < length l ->
  In v (vsmerge (selN (vsupd_vecs l avl) a valuset0)).
Proof.
  intros.
...
Proof.
  intros.
  eapply selN_Forall2 with (da := valuset0) (db := valuset0).
...
Lemma vssync_selN_not_in : forall l i d,
  ~ In i l ->
  selN (vssync_vecs d l) i valuset0 = selN d i valuset0.
Proof.
...
Lemma vssync_vecs_selN_In : forall l i d,
  In i l ->
  i < length d ->
  selN (vssync_vecs d l) i valuset0 = (fst (selN d i valuset0), nil).
Proof.
...
Proof.
  ...
  apply forall_incl_refl.
  rewrite vssync_vecs_vssync_comm.
  rewrite <- updN_selN_eq with (ix := a) (l := vs) (default := valuset0) at 2.
  apply forall2_updN.
  apply IHl; auto.
  eapply Forall_cons2; eauto.
...
Definition possible_crash_list l (l': list tagged_block) :=
  length l = length l' /\
  forall i, i < length l -> In (selN l' i tagged_block0) (vsmerge (selN l i valuset0)).

Lemma synced_list_selN : forall l i def,
  selN (synced_list l) i (def, nil) = (selN l i def, nil).
Proof.
  unfold synced_list; intros.
  unfold valuset0, valuset in *.
  rewrite selN_combine.
...
Lemma possible_crash_list_unique : forall a b,
  (forall n, snd (selN a n valuset0) = nil) ->
...
Proof.
  ...
  generalize H2.
  unfold synced_list, valuset0, valuset in *; rewrite selN_combine; eauto.
  rewrite repeat_length; auto.

  generalize H2.
  unfold synced_list, valuset0, valuset in *; rewrite selN_combine; simpl.
...
Proof.
  ...
  unfold valuset0, valuset in *; rewrite selN_combine; auto.
  rewrite repeat_length; auto.
Qed.
...
Section ArrayCrashXform.
  ...
  Lemma crash_xform_arrayN_combine_nils: forall (l : list tagged_block) st,
  ...
  Lemma crash_xform_arrayN_synced: forall (l : list tagged_block) st,
