...
Theorem log_inc_two_body_ok: forall xp s0 s1 mscs,
  ...
  POST RET:mscs   exists m', MEMLOG.rep xp (ActiveTxn mbase m') mscs *
...
Theorem log_inc_two_recover_ok: forall xp s0 s1 mscs cachesize,
  ...
  POST RET:^(mscs, r)
                  ...
                  [[ (s0 |-> (v0 ^+ $1) * s1 |-> (v1 ^+ $1) * F)%pred (list2mem m') ]]
  REC RET:^(mscs, fsxp)
                  MEMLOG.rep fsxp.(FSXPMemLog) (NoTransaction mbase) mscs \/
                  exists m', MEMLOG.rep fsxp.(FSXPMemLog) (NoTransaction m') mscs *
                  [[ (s0 |-> (v0 ^+ $1) * s1 |-> (v1 ^+ $1) * F)%pred (list2mem m') ]]
