...
Proof.
  ...
  hoare; apply MEMLOG.activetxn_would_recover_old.
Qed.
...
Proof.
  unfold log_inc_two.
  hoare.
  rewrite MEMLOG.notxn_would_recover_old.
  cancel.
  rewrite MEMLOG.activetxn_would_recover_old.
  cancel.
Qed.
...
Theorem log_inc_two_recover_ok: forall xp s0 s1 mscs,
  ...
  REC RET:^(mscs, xp)
    ...
    exists m', MEMLOG.rep xp Fm (NoTransaction m') mscs *
               [[ (s0 |-> (v0 ^+ $1) * s1 |-> (v1 ^+ $1) * F)%pred (list2mem m') ]]
  >>} log_inc_two xp s0 s1 mscs >> MEMLOG.recover.
Proof.
  intros.
