...
Definition delete1 dnum name :=
  fsxp <- Get mFsxp;
    mscs <- Get mMscs;
    r <- CFS.delete fsxp dnum name mscs;
    match r with
    | Some r =>
      let '(mscs', (r, _)) := r in
      _ <- Assgn mMscs mscs';
        ...
        _ <- match r with
            | OK _ =>
              let up := DIRTREE.delete_from_dir name in
              var_update vDirTree (DIRTREE.alter_inum dnum up)
            | _ => Ret tt
            end;
        Ret (value r)
...
Proof.
  unfold delete1, wrap_syscall; intros.
  step.
  step.
  step.
